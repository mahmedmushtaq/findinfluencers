...
import styles from "../../../../../../../styles/scss/chats/MessageBox.module.scss";

const MessageBox = ({ chat }) => {
  const user = useSelector((store) => store.user);
  const [loading, setLoading] = useState(false);
  const { scrollBottom, senderTyping, socket } = useSelector(
  ...
  useEffect(() => {
    setTimeout(() => {
      scrollManual(Math.ceil(msgBox.current.scrollHeight * 0.05));
    }, 100);
  }, [scrollUp]);
    ...
    >
      ...
      {senderTyping.typing && senderTyping.chatId === chat.id ? (
        <div className={styles["message"]}>
          <div className={styles["other-person"]}>
            <p className="m-0">
